#!/usr/bin/env python3
"""
pipeline_window_complete.py  –  checks whether the xmit instruction COMPLETES
inside the speculative window defined by:

    last_committed.retire < xmit.complete < first_noncommitted.retire

Uses xmit.complete instead of xmit.issue so that instructions which issue
speculatively but are then stalled/re-queued by STT (in lsq_unit.cc) are not
counted as hits.

If first_noncommitted is squashed (retire==0), the upper bound is infinity.

Usage:
    python3 pipeline_window_complete.py <input_dir> [--jobs N] [--out results.json] [--scheme N]
"""
...
def check_window(trace_path, xmit_pc, lc_pc, fnc_pc):
    """
    Check: lc_retire < xmit.complete < fnc_retire
    Uses complete instead of issue so STT-stalled loads aren't counted as hits.
    ...
    # window: lc_retire < xmit.complete < fnc_retire
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=None,
                    help="Only process the first N tests (sorted by name)")
    args = ap.parse_args()
    ...
    print(f"gem5 scheme={_scheme}  jobs={args.jobs}  window=complete\n")
    ...
    print(f"\nResults: {yes} completed in window  |  {no} did not  |  {err} error(s)  (total {len(results)})")
