...
one sig IX4 {} -- spo position 3 (last)

// single concrete instruction sig
sig Instruction {
	kind: one InstrType,
	idx: one (IX0 + IX1 + IX2 + IX3 + IX4),  -- bijection to fixed position atoms
...
fact idx_surjective { (IX0 + IX1 + IX2 + IX3 + IX4) = Instruction.idx }

-- IX0 < IX1 < IX2 < IX3 is a fixed static ordering (no atom permutations possible)
pred lt_ix[a, b: univ] {
	(a=IX0 and b in IX1+IX2+IX3+IX4) or
	(a=IX1 and b in IX2+IX3+IX4) or
	(a=IX2 and b in IX3+IX4) or
	(a=IX3 and b=IX4)
}
...
run gen_lit {
  gen_useful_litmus
} for 8 but exactly 5 Instruction, exactly 1 rBool, exactly 1 cBool, exactly 1 tBool
...
fun leakage_function : Operand {Loads.inaddr}
