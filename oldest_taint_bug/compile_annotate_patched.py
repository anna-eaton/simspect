...
def compile_to_x86(ll_path: Path, out_dir: Path) -> tuple[Path, Path]:
    ...
    for flag, out in [("-S", s_path), ("-c", o_path)]:
        r = subprocess.run(
            ["clang-15", "--target=x86_64-unknown-linux-gnu", "-O1",
...
def disassemble_instrs(binary: Path, func_base: int) -> list[tuple[int, str, str]]:
    ...
    for line in r.stdout.splitlines():
        ...
        if f"<{binary.name}>:" in line:
            in_func = True
            continue
        # stop at the next top-level function symbol.
        # Embedded labels generated by our inline asm (e.g. __litmus_..._pcN)
        # appear at column 0 in objdump output just like function symbols, but
        # they are still inside the current function — skip them.
        if in_func and re.match(r'^[0-9a-f]+ <', line):
            if '__litmus_' in line:
                continue  # embedded PC marker — stay in function
            break
        if in_func:
