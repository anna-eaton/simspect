...
-- fix ordering duplication, eg each load must be numbered in PO
-- (util/ordering on subtypes doesn't work with variable-count sigs;
--  use find_duplicates.py post-hoc instead)
// open util/ordering[Instruction] as IOrd  -- blocked: abstract sig in translator
...
fact { Instruction = Load + Store + Branchx + Branchn + Otherx + Othern }
// instruction sigs
sig Instruction {	
...
/*********************************************************************************
 * Symmetry breaking on order
 */

-- fact break_symmetry { ... }  -- disabled: waiting on util/ordering fix
