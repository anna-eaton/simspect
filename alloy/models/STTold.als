...
// EDIT THESE
fun speculation_contract_p[p: PTag->univ] : Instruction {i_p[p] - (retired.rBool)} // example : all retired instructions are non-spec
fun speculation_contract_p[p: PTag->univ->univ] : Instruction {Instruction - (retired.rBool+p[RB].rBool)} // example : all retired instructions are non-spec
fun hardware_protection_policy_p[p: PTag->univ] : State {Mem_s - p[RS]} // example : all regs (TODO was this supposed to be mem)
...
fun leakage_function_p[p: PTag->univ->univ] : Instruction -> State {xmit_p2[p]} // example : all transmitters
...
let gen_useful_litmus {
  ...
  all i: Instruction | secure_speculation_scheme_p1[RI->i]
  all s: State | secure_speculation_scheme_p1[RS->s]
  all i: Instruction, s: State |
    (i -> s in addrop) => secure_speculation_scheme_p2[RA->i->s]
  all i: Instruction, s: State |
    (i -> s in dataop) => secure_speculation_scheme_p2[RD->i->s]
  all i: Instruction, s: State |
    (i -> s in outreg) => secure_speculation_scheme_p2[RR->i->s]
  all i: Instruction, s: State |
    (i -> s in outmem) => secure_speculation_scheme_p2[RM->i->s]

  // right now this is just touching retired, can mess with other booleans later.
  all b: first_unretired | secure_speculation_scheme_p2[RB->b->rBool]
}
...
run gen_lit {
  gen_useful_litmus
} for 2 //for 5 but exactly 3 Instruction
