...
fun load_dep_p[i : Instruction, p: PTag->univ->univ] : State -> State {s_p[p] <: (~(inaddr-p[RA])).(iden :> ((i & Load)-uu[p][RI])).(outreg-p[RR])+((Mem_s - uu[p][RS]) -> ((i & Load) - uu[p][RI]).(outreg-p[RR])) :> s_p[p]} 
// stores depend mem loc on data op and addr op (did it differently but should do same thing)
fun store_dep_p[i : Instruction, p: PTag->univ->univ] : State -> State {s_p[p] <: ~(inaddr-p[RA]+ inreg-p[RD]).(iden :> ((i & Store)-uu[p][RI])).(outmem-p[RM]) :> s_p[p]}
// div depends outreg on inreg and inaddr
fun div_dep_p[i : Instruction, p: PTag->univ->univ] : State -> State {s_p[p] <: ~(inaddr-p[RA] + inreg-p[RD]).(iden :> ((i & Div)-uu[p][RI])).(outreg - p[RR]) :> s_p[p]}
// other depends outreg and outmem on inreg and inaddr
fun other_dep_p[i : Instruction, p: PTag->univ->univ] : State -> State {s_p[p] <: ~(inaddr + inreg-p[RA]-p[RD]).(iden :> ((i & Other)-uu[p][RI])).(outreg + outmem  - p[RR] - p[RM]) :> s_p[p]}
...
fun xmit_p[p: PTag->univ->univ] : Instruction -> State {i_p[p] <: (((Load+Store+Ctrl+Div) <: (inaddr-p[RA]))+((Ctrl+Div) <: (inreg-p[RD]))) :> s_p[p]}
